...
def _detect(redis, ip: str, method: str, path: str, ua: str,
            payload_size: int, status_code: int) -> list[tuple[str, int]]:
    ...
    try:
        # --- Rate-based checks ---
        rapid_count = _incr(redis, f"bot:rapid:{ip}", RULES["RAPID_FIRE"]["window"])
        if rapid_count > RULES["RAPID_FIRE"]["threshold"]:
            hits.append(("RAPID_FIRE", RULES["RAPID_FIRE"]["score"]))

        burst_count = _incr(redis, f"bot:burst:{ip}", RULES["BURST_TRAFFIC"]["window"])
        ...
        if method == "POST":
            post_count = _incr(redis, f"bot:post:{ip}", RULES["EXCESSIVE_POST"]["window"])
        ...
        total_count = _incr(redis, f"bot:total:{ip}", RULES["HIGH_ERROR_RATE"]["window"])

        if status_code >= 400:
            err_count = _incr(redis, f"bot:errors:{ip}", RULES["HIGH_ERROR_RATE"]["window"])
            consec = _incr(redis, f"bot:consec:{ip}", 60)
...
def _accumulate_risk(redis, ip: str, score: int) -> int:
...
class BotDetectionMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        cumulative = _accumulate_risk(redis, ip, top_score)
        ...
        log_request(
            ...
            method=method,
            action=action,
            detection_type=f"bot:{top_name.lower()}",
            score=score,
            reasons=all_names,
        )
